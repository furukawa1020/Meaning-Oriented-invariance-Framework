...
\end{equation}
where gravity was subtracted per-axis before summing. The proxy index $\rho_{LC} = |\mathrm{Spearman}(y, \mathrm{SMA})|$ was computed subject-wise over $\sim$60--90 sliding windows per subject (50\% overlap).
...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Aspect} & \textbf{Without Audit} & \textbf{With LabelScope} \\
\midrule
Reported metric & Classification performance & Performance metric + $L_{Final} = 2$ \\
Potential interpretation & ``Stress detector'' & Task-condition proxy \\
...
\midrule
ACC (Wrist) & SMA & $\frac{1}{T}\int_0^T(|a_x|+|a_y|+|a_z|)dt$; gravity-subtracted per-axis. \\
\bottomrule
\end{tabular}
\end{table}
...
The WESAD and SWELL-KW datasets are publicly available. We support reproducible auditing under fixed preprocessing parameters and permutation seeds. The implementation uses NeuroKit2 (v0.2.1) and cvxEDA. Code for the demonstrations will be made available upon publication.
