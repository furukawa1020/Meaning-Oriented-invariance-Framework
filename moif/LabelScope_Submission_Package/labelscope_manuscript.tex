...
\toprule
\textbf{Window} & \textbf{N} & \textbf{D} & \textbf{$T_{obs}$} & \textbf{$\bar{T}_{null}$} & \textbf{SD} & \textbf{$\hat{p}$} & \textbf{$L_{SA}$} \\
\midrule
...
\IF{$\hat{p}_{shuf} \ge \alpha$}
    \STATE $L_{SA} \gets 1$ \COMMENT{Noise/artifact cap}
\ELSE
...
\midrule
Reported metric & Classification performance & Accuracy + $L_{Final} = 2$ \\
...
\end{enumerate}
This proactive auditing reduces the risk of misdirected interventions and improves the overall trust in adaptive human-machine systems.
