...
Let $X \in \mathbb{R}^{N \times D}$ be a physiological feature matrix derived \textit{exclusively} from physiological modalities (ECG and EDA). Let $y \in \mathbb{R}^N$ be the operational label, and $C_B \in \mathbb{R}^{N \times K}$ be a matrix of $K$ behavioral covariates (e.g., accelerometer-derived features). This separation is structural: $X$ and $C_B$ are always kept disjoint. LabelScope evaluates the quadruple $(X, y, C_B, S)$ and returns a final claim level $L_{Final} \in \{0, 1, 2, 3\}$.
...
\subsection{Audit Module 4: Claim Audit and Final Level}
...
\toprule
\textbf{N win.} & \textbf{D feat.} & \textbf{k} & \textbf{B} & \textbf{$T_{obs}$} & \textbf{$\hat{p}_{shuf}$} & \textbf{$L_{SA}$} \\
\midrule
1110 & 16 & 50 & 1000 & 0.71 & 0.001 & 3 (pass) \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Interpretation}: Passing the SA indicates non-random label organization in the tested physiological feature representation, but does not by itself establish movement-independent internal-state validity. The claim is still bounded at Level 2 by the PA, since the SMA covariate ($\rho_{LC} = 0.65 > \tau_{PA}$) flags a task/protocol-linked movement association. This demonstrates the necessity of the multi-module framework: $L_{Final} = \min(3,\,2,\,3) = 2$.
...
The core logic of the LabelScope audits is formalized in Algorithms 1--3. These procedures ensure that the audit results are deterministic and independent of the specific machine learning model used in the downstream task. For the experimental demonstrations, the following hyperparameters were utilized: k-NN neighborhood size $k=50$, number of permutations $B=1000$, and block-shuffling length $L=60$ seconds.
...
\begin{algorithmic}[1]
\REQUIRE Label vector $y$, Behavioral covariate matrix $C_B \in \mathbb{R}^{N \times K}$, Threshold $\tau_{PA}$
\ENSURE $L_{PA} \in \{2, 3\}$, proxy index $\rho_{LC}$
\FOR{$k = 1$ \TO $K$}
    \STATE $\rho_k \gets |\text{Spearman}(y, c_k)|$ \COMMENT{Subject-wise, then averaged}
\ENDFOR
\STATE $\rho_{LC} \gets \max_k \rho_k$
\IF{$\rho_{LC} > \tau_{PA}$}
...
Table \ref{tab:features} lists the physiological features used as the feature matrix $X$ in the Structure Audit ($D = 16$). All features were extracted from ECG and EDA signals using a 60-second sliding window with 50\% overlap. Accelerometer-derived features (SMA, Mean Accel) were used exclusively as behavioral covariates $C_B$ in the Proxy Audit and are listed separately in Table \ref{tab:covariates}.

\begin{table*}[!ht]
\caption{Physiological Features Used in Structure Audit ($X$, $D = 16$)}
\label{tab:features}
\centering
\begin{tabular}{@{}lll@{}}
\toprule
...
\begin{table}[!ht]
\caption{Behavioral Covariates Used in Proxy Audit ($C_B$, $K = 2$)}
...
ACC (Wrist) & Mean Accel & Average per-axis acceleration magnitude across $x$, $y$, $z$. \\
\bottomrule
\end{tabular}
\end{table}
