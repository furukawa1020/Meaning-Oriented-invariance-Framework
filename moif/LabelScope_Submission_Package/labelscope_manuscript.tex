...
We introduce \textbf{LabelScope}, a claim-relative audit framework that caps the strongest defensible HMS inference supported by an operational physiological label before predictive accuracy is interpreted. By treating validity as a claim-relative property, LabelScope allows researchers to formally specify the strongest defensible interpretation of their models, whether as a coarse behavioral proxy or a fine-grained internal state monitor.
...
\textbf{Level 3} & Candidate Support & Passes all audits & Candidate support for physiology-based state inference under the tested resolution, feature representation, and observed covariates. & Absolute ground truth or unconfounded psychological-state measurement. \\
\bottomrule
\end{tabular}
\end{table*}
...
To illustrate the utility of the LabelScope framework, we applied it to two public physiological datasets: SWELL-KW and WESAD. For each dataset, we defined an intended HMS claim representing a typical research goal and applied LabelScope (Algorithms 1--3) to audit the supportability of the operational labels under the demonstration thresholds $\tau_{RA} = 5.0$, $\tau_{PA} = 0.30$, $\alpha = 0.05$, $k = 50$, $B = 1000$, and block length $L = 60$\,s. The accelerometer-derived SMA was used as the sole behavioral covariate ($K = 1$) in the Proxy Audit. The physiological feature matrix $X$ for the Structure Audit contained only ECG- and EDA-derived features ($D = 14$; see Appendix).
...
\end{enumerate}
In adaptive systems, the audited level can be used as a guard condition: Level 0 blocks real-time adaptation, Level 2 permits behavior-aware interventions but blocks internal-state interventions, and Level 3 permits cautious state-contingent adaptation under observed-covariate limits. This proactive auditing reduces the risk of misdirected interventions and improves the overall trust in adaptive human-machine systems.
