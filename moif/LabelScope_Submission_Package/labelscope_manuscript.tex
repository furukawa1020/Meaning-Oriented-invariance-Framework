...
\textbf{Level 3} & Structural Ambiguity & Local neighborhood mixing & Recognition of broad state contrasts (e.g., high vs. low arousal). & Precise, deterministic regression of continuous state. \\
\textbf{Level 4} & Candidate Valid State & Passes all audits & Strong evidence for physiology-grounded state detection. & Absolute ground truth (unobserved confounders may exist). \\
\bottomrule
\end{tabular}
\end{table*}
...
\subsubsection{Proxy Audit (PA)}
The intended claim was continuous sensing of internal stress. However, many of the stress-induction tasks (e.g., public speaking, mental arithmetic) were associated with changes in posture and physical activity. We audited the binary stress vs. non-stress label against the accelerometer-based Signal Magnitude Area (SMA) derived from the wrist-worn Empatica E4.

\textbf{Result}: The Proxy Audit was calculated subject-wise using a 60-second sliding window. Because the relationship between physiological activation and movement is non-linear but monotonic, we utilized Spearman's rank correlation. Across the 15 subjects, the mean proxy index was $\rho_{LC} = 0.65$ ($\sigma = 0.12$, 95\% CI $[0.59, 0.71]$), indicating a strong material association between the label and physical movement.
\textbf{Interpretation}: The claim was \textbf{Capped at Level 2}. The system must be reported as a "Task-Condition Proxy" detector. While accurate, the model is likely sensing the physical manifestations of the task rather than a pure internal physiological state.
...
\appendices
\section{Comprehensive Feature Definitions}

Table \ref{tab:features} provides a complete listing of the physiological features utilized in the Structure Audit (SA). All features were extracted using a 60-second sliding window with 50\% overlap.
