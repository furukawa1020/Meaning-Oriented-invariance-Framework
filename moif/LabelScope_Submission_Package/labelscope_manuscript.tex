...
\end{equation}
If $R_P$ exceeds a conservative threshold (e.g., $\tau_{RA} = 5.0$), the claim is capped at Level 0. This ensures that the modeling resolution matches the "information frequency" of the labels.
...
\end{equation}
If $\hat{p}_{shuf}$ fails to achieve statistical significance (e.g., $\alpha = 0.05$), the claim is capped at Level 1. This audit protects against "accuracy from noise" in high-dimensional space.
...
\textbf{Result}: The empirical p-value $\hat{p}_{shuf}$ was significant ($p < 0.05$), meaning the "stress vs. non-stress" labels passed the SA and avoided a Level 1 cap. 

\textbf{Interpretation}: While the physiological structure is non-random (passing SA), the claim remains bounded at Level 2 by the PA due to the behavioral confounding. This highlights the necessity of the multi-module framework: passing SA proves physiological coherence, but only PA can flag when that coherence is driven by physical activity.
