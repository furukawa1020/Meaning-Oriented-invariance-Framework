...
def run_conflict_audit():
    ...
    for i in range(len(full_df)):
        ...
        if row['is_HA'] or row['is_LA']:
            ...
            act_conflict = (n_cats != self_cat).mean() if len(n_cats) > 0 else np.nan
        ...
        if row['is_HA'] and (row['is_PV'] or row['is_NV']):
            ...
            mean_conflict = (n_v_cats != self_v_cat).mean() if len(n_v_cats) > 0 else np.nan
    ...
    for _ in range(n_boot):
        idx = np.random.choice(len(res_df), len(res_df), replace=True)
        boot_res = res_df.iloc[idx]
        gap = boot_res['mean_conflict'].mean() - boot_res['act_conflict'].mean()
        boot_gaps.append(gap)
        
    report = {
        "mean_activation_conflict": float(res_df['act_conflict'].mean()),
        "mean_meaning_conflict": float(res_df['mean_conflict'].mean()),
        "conflict_gap": float(res_df['mean_conflict'].mean() - res_df['act_conflict'].mean()),
        "gap_95_ci": [float(np.percentile(boot_gaps, 2.5)), float(np.percentile(boot_gaps, 97.5))],
        "verdict": "M2_FAIL" if (res_df['mean_conflict'].mean() - res_df['act_conflict'].mean()) < 0.10 else "M2_AUDIT_CONTINUE"
    }
    
    with open(os.path.join(OUT_DIR, "phase_m2_gate_report_v2.json"), "w") as f:
        json.dump(report, f, indent=4)
    print("Phase M2 Gate Report V2 Generated.")
