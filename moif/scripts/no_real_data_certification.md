...
To operationally prevent unintended real-data access, the following guard will be placed at the top of the main evaluation script `moif_hms_eval.py` before Phase 2 begins:
...
if os.environ.get("ALLOW_REAL_DATA_PHASE_2A_INGESTION") != "1":
    print("FATAL: Real data access is locked.")
    print("You must explicitly pass ALLOW_REAL_DATA_PHASE_2A_INGESTION=1 to execute Phase 2A.")
    sys.exit(1)
```

This guard reduces the risk of accidental outcome-dependent changes by requiring explicit authorization before real-data access.
